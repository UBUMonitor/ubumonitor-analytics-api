...
public class PersistenceFetchAdapter implements PersistenceFetchPort {
  ...
  public UserEnrolledCourses getEnrolledCourses(Integer userId) {
    Map<Integer, UsersCoursesRecord> byCourseId =
        jooq.dsl()
            .selectFrom(USERS_COURSES)
            .where(USERS_COURSES.USER_ID.eq(userId))
            .fetch()
            .intoMap(USERS_COURSES.COURSE_ID);

    List<CoursesRecord> coursesRecords =
        jooq.dsl().selectFrom(COURSES).where(COURSES.ID.in(byCourseId.keySet())).fetch();

    List<Enrollment> enrollments =
        coursesRecords.stream().map(c -> mapper.toDomain(c, byCourseId.get(c.getId()))).toList();

    return UserEnrolledCourses.builder().userId(userId).enrolledCourses(enrollments).build();
  }
}
