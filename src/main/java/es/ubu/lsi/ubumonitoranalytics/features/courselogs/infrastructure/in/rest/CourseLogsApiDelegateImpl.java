...
public class CourseLogsApiDelegateImpl implements CourseLogsApiDelegate {
    ...
    @Override
    public ResponseEntity<CourseLogsInfoResponseDto> getCourseLogs(Integer courseId, CourseLogsInfoRequestDto courseLogsInfoRequestDto) {
        CourseLogsInfoRequest courseLogsInfoRequest =  getCourseLogsMapper.toDomain(courseId, courseLogsInfoRequestDto);
        FetchCourseLogsResult fetchCourseLogsResult = fetchCourseLogsUseCase.getCourseLogs(courseLogsInfoRequest);
        return ResponseEntity.ok(getCourseLogsMapper.toDto(fetchCourseLogsResult));
    }

    @Override
    public ResponseEntity<CourseLogsMetricsResponseDto> getCourseLogsMetrics(Integer courseId, CourseLogsMetricsRequestDto courseLogsMetricsRequestDto) {
        CourseLogsMetricsRequest courseLogsMetricsRequest = getCourseLogsMetricsMapper.toDomain(courseId, courseLogsMetricsRequestDto);
        CourseLogsMetricsResult result = fetchCourseLogsUseCase.getCourseLogsMetrics(courseLogsMetricsRequest);
